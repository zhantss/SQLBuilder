...
function initialization(store: Store<{}>) {
    ...
    let init = null;
    if (params && params[window.SQLBuilder.urlrequired]) {
        try {
            init = params[window.SQLBuilder.urlrequired];
            if (init.modelId) {
                store.dispatch({ type: "SYNC_SYNC", info: cn.sync_dialog_info });
                axios.post(window.SQLBuilder.url.get_model_init, { modelId: init.modelId })
                    .then(response => {
